...
public interface CampaignRepository extends JpaRepository<Campaign, String>,PagingAndSortingRepository<Campaign, String> {
    ...
    static final String QUERY_FIND_CAMPAIGN =
        "select                                                                                                         "
            ...
            + "join \"source\" s2                                                                                       "
            + "on                                                                                                       "
            + "        s2.id_source = s.source_id_source                                                                "
            + "where                                                                                                    "
            + "        (:source is null or UPPER(s2.id_source) = UPPER(cast( :source as text)))                         "
            + "        and (:period is null or UPPER(c.\"period\") = UPPER(cast( :period as text)))                     ";
    
    static final String CLAUSE_YEAR = " and (:year is null or s.\"year\" = :year)                                       ";
